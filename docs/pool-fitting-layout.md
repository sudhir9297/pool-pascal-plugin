...
Skimmers use the actual polygon area. One or two target the +X end, the deep end for this plugin's sloped pools. Larger groups target a long side. Skimmers remain on the selected outward-facing side. Return inlets are distributed around the full perimeter, with clearance from skimmers, so each wall contributes to circulation. If the outline cannot fit the calculated quantities, Pool options reports the shortage instead of silently stacking fittings. Other generated wall fittings use a 0.65 m center clearance chosen for the default asset sizes. These layout choices do not model prevailing wind or certify circulation, corner coverage, maximum return gaps or compliance with every clause of the cited public-pool rule.
