...
export type PoolSpilloverPlacement = Pick<PoolSpilloverNode,
  'position' | 'rotation' | 'sourcePoolId' | 'targetPoolId' | 'intersection' | 'sourceSide' | 'width' | 'length' | 'dropHeight' | 'waterColor' | 'surfaceColor'
>
...
/** Finds a shared spillway band even when the two pool shells have a small gap. */
function findAdjacentSpillway(first: PoolNode, second: PoolNode) {
  const left = worldPolygon(first)
  const right = worldPolygon(second)
  let best: { gap: number; width: number; position: PoolPoint; tangent: PoolPoint } | null = null
  for (let i = 0; i < left.length; i += 1) {
    ...
    for (let j = 0; j < right.length; j += 1) {
      ...
      if (gap > 0.75 || (best && (gap > best.gap || (gap === best.gap && width < best.width)))) continue
      ...
      best = { gap, width, position, tangent: [tangent[0], tangent[1]] }
    }
  }
  if (!best) return null
  const halfWidth = best.width * 0.45
  const halfLength = Math.max(0.06, best.gap / 2)
  ...
  return { position: [p[0], 0, p[1]] as [number, number, number], rotation: [0, Math.atan2(best.tangent[0], best.tangent[1]), 0] as [number, number, number], length: Math.max(0.12, best.gap), width: Math.max(0.3, best.width * 0.9), intersection }
}

/** Resolves a directional spillover using two nearby pool polygons. */
export function resolvePoolSpillover(first: PoolNode, second: PoolNode): PoolSpilloverPlacement | null {
  if (first.parentId !== second.parentId) return null
  const joint = findSharedPoolJoint(first, second) ?? findAdjacentSpillway(first, second)
  ...
  return {
    ...
    sourceSide,
    width: Math.max(0.3, joint.width),
