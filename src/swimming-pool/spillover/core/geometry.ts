...
export function buildPoolSpilloverGeometry(node: PoolSpilloverNode, waterStyle?: WaterfallWaterStyle) {
  ...
  const targetX = node.connectionMode === 'overlap'
    ? sourceBoundaryX + flowDirection * overlapReach
    : rawTargetX
  ...
  if (node.connectionMode === 'overlap') {
    const surface = new Mesh(new BoxGeometry(
      channelLength,
      node.lipThickness,
      width,
      1, 1, 40,
    ), surfaceMaterial())
    deformGeometry(surface.geometry, (x, y, z) => [
      x + sampleSpilloverEdge(node.sourceEdge, z), y, z,
    ])
    surface.name = 'pool-spillover-overlap-surface'
    surface.position.set(channelCenter, -node.lipThickness, (sourceBoundaryZ + targetZ) / 2)
    group.add(surface)
  ...
  if (node.connectionMode === 'channel') {
    ...
    leftWall.position.set(channelCenter, -node.lipThickness / 2 + wallHeight / 2 + 0.001, (sourceBoundaryZ + targetZ) / 2 - Math.max(0, wallOffset - wallInset))
    group.add(leftWall)
    ...
    rightWall.position.set(channelCenter, -node.lipThickness / 2 + wallHeight / 2 + 0.001, (sourceBoundaryZ + targetZ) / 2 + Math.max(0, wallOffset - wallInset))
    group.add(rightWall)
    for (const side of [-1, 1] as const) {
      const border = new Mesh(new BoxGeometry(channelLength, node.lipThickness, node.lipThickness), surfaceMaterial())
  ...
  deformGeometry(sheetGeometry, (x, y, z) => {
    ...
    return [x, y, z + flowDirection * offset + waterLandingInset]
  })
